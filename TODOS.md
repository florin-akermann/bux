...
The file says why: version 0.1 has no way to write a number out.
That reason is gone.
...
A reader who opens the file must see what everyday Lumen shows somebody.
[057][a] - `example/main.lm` imports `io` and writes each answer it works out with `io.println`.
[057][b] - `docs/specs/example-program.md` says what the run writes, not why it wrote nothing.
[057][c] - `README.md` says the same, and the test asserts the output as well as the exit code.
