...
`Err` holds what a throw said of itself, and a parameter may also be a `List`, which crosses as a
`java.util.List` of what it holds. `Int` compiles to a `long`, and `int` or `char`
