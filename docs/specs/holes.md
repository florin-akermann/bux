...
A hole is accepted by `lumen check` and refused by `lumen build`.
...
Lumen hides no name, so a module that declares its own `todo` is refused with `L0302`, and a
...
## What `lumen check` does

Nothing.
A hole is well typed, so `lumen check` accepts a module holding one and exits `0`.
...
`lumen run` builds before it runs, so it refuses the same way.

Nothing is written when a module holds a hole: the refusal comes before any class file.

## Why the build and not the check

`lumen check` is what a reader runs while writing, and unfinished work is the state it is written
in.
...
`lumen build` is what produces something that runs.
...
`L0600` is raised by `lumen build` and `lumen run`, never by `lumen check` or `lumen fmt`.
