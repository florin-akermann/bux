...
- A carve-out (`[ignore]`, `default_skip`, `[disable]`, a lint allowed in `Cargo.toml`) is a debt.
- A debt carries its reason beside it; a bare `#[allow]` in source is refused.
...
- `crates/cli/` is the binary: argument parsing and help only, no compiler logic.
...
- Each crate has one integration-test binary, `tests/integration/main.rs`, with `mod` per file.
...
- A crate is added only when it cannot be replicated at acceptable cost, marked `# approved:`.
