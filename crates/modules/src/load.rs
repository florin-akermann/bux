...
impl Loader {
    /// Reads `name` and everything it imports, putting it below every module it reaches.
    ...
    fn follow(&mut self, import: &Import, from: &Path, source: &str) -> Result<(), NotLoaded> {
        ...
        if !beside.is_file() {
            let missing = LoadErrorKind::NoSuchModule(name.to_owned());
            return Err(refusal(
                &LoadError::at(import.module.span, missing),
                from,
                source,
            ));
        }
