...
/// Where the bytes `second` first differ from the bytes `first`, which is `None` where they
/// agree, and the length of the shorter where one holds the other and more.
pub fn first_difference(first: &[u8], second: &[u8]) -> Option<usize> {
    let differs = first.iter().zip(second).position(|(a, b)| a != b);
    let shorter = first.len().min(second.len());
    differs.or_else(|| (first.len() != second.len()).then_some(shorter))
}

/// Every file that ends in `.<extension>` under each of `directories`, however deep, by its path
/// from `root`, in the order the paths sort.
pub fn files_ending_in(extension: &str, root: &Path, directories: &[&str]) -> Vec<String> {
    let mut found = Vec::new();
    for directory in directories {
        gather(root, Path::new(directory), &mut found);
    }
    found.retain(|path| {
        Path::new(path)
            .extension()
            .is_some_and(|ending| ending == extension)
    });
    found.sort();
    found
}

/// Every file under `directory`, however deep, by its path from `root`.
fn gather(root: &Path, directory: &Path, found: &mut Vec<String>) {
    for entry in fs::read_dir(root.join(directory)).expect("a directory to walk is readable") {
