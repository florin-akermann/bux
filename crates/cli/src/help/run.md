...
module, so running the module class with `java --enable-preview` directly does exactly what
`lumen run` does. The flag is there because every class Lumen writes is a value class, which JDK
28 holds in preview.
