...
holds it to canonical form, parses it, resolves every name in it, and gives every expression a
type. Nothing is written back.
...
Type inference comes last. A type written where another is needed, a call with the wrong number
...
documents a boundary rather than on every line. `lumen explain` says more about any code that is
printed.
