//! The parser's printed form: a tree one line per node, or an error as a fixture holds it.
//!
//! [`render`] writes `<indent><node> <start>..<end>` for each node, which is the format of a
//! `tests/spec/parser/<name>.ast` file, and [`render_error`] writes a `<name>.error` file.
//! The parser's tests read it, and so does the harness of the parser written in Bux,
//! `crates/cli/tests/integration/bux_parser.rs`, which includes this file by its path.
...
fn expr_node(tree: &mut Tree, depth: usize, expr: &Expr) {
    ...
    match &expr.kind {
        ...
        ExprKind::String(value) => tree.node(depth, &format!("string {}", quoted(value)), span),
...
fn pattern_node(tree: &mut Tree, depth: usize, pattern: &Pattern) {
    ...
    match &pattern.kind {
        ...
        PatternKind::String(value) => {
            tree.node(depth, &format!("pattern-string {}", quoted(value)), span);
        }
...
/// `text` between quotes, with `"`, `\`, and each ASCII control character escaped.
///
/// The printed form escapes only these, so one spelling holds for any text on both printers.
/// `compiler/ast.lm` writes a string the same way, and `docs/specs/grammar.md` states it.
fn quoted(text: &str) -> String {
    let mut written = String::from("\"");
    for character in text.chars() {
        match character {
            '"' | '\\' => written.extend(['\\', character]),
            '\n' => written.push_str("\\n"),
            '\t' => written.push_str("\\t"),
            '\r' => written.push_str("\\r"),
            '\0' => written.push_str("\\0"),
            control if control.is_ascii_control() => {
                let _ = write!(written, "\\u{{{:x}}}", u32::from(control));
            }
